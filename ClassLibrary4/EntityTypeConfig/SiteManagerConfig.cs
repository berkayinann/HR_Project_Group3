...
{
    public class SiteManagerConfig : BaseEntityConfig<SiteManager>
    {
        public override void Configure(EntityTypeBuilder<SiteManager> builder)
        {
            ...
            builder.HasOne(x => x.Job).WithMany(x => x.SiteManagers).HasForeignKey(x => x.JobId);

            builder.HasOne(x => x.AppUser).WithOne(x => x.SiteManager).HasForeignKey<SiteManager>(x => x.AppUserId);
